import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:home_ease/core/theming/colors.dart';

class TextStyles {
  static TextStyle font48Black700 = TextStyle(
    fontSize: 48.sp,
    fontWeight: FontWeight.w700,
    color: ColorsApp.black,
    height: 1,
  );

  static TextStyle font34Black700 = TextStyle(
    fontSize: 34.sp,
    fontWeight: FontWeight.w700,
    color: ColorsApp.black,
  );

  static TextStyle font10Bgray500 = TextStyle(
    fontSize: 10.sp,
    fontWeight: FontWeight.w500,
    color: ColorsApp.gray,
  );
  static TextStyle font28Black700 = TextStyle(
    fontSize: 28.sp,
    fontWeight: FontWeight.w700,
    color: ColorsApp.black,
  );
  static TextStyle font24Black700 = TextStyle(
    fontSize: 24.sp,
    fontWeight: FontWeight.w700,
    color: ColorsApp.black,
  );

  static TextStyle font12white700 = TextStyle(
    fontSize: 12.sp,
    fontWeight: FontWeight.w700,
    color: ColorsApp.white,
  );

  static TextStyle font18yallow700 = TextStyle(
    fontSize: 18.sp,
    fontWeight: FontWeight.w700,
    color: ColorsApp.yallow,
  );

  static TextStyle font18Black700 = TextStyle(
    fontSize: 18.sp,
    fontWeight: FontWeight.w700,
    color: ColorsApp.black,
  );
  static TextStyle font14Black600 = TextStyle(
    fontSize: 14.sp,
    fontWeight: FontWeight.w600,
    color: ColorsApp.black,
  );
  static TextStyle font14Bgray500 = TextStyle(
    fontSize: 14.sp,
    fontWeight: FontWeight.w500,
    color: ColorsApp.gray,
  );

  static TextStyle font14Bgreengray500 = TextStyle(
    fontSize: 14.sp,
    fontWeight: FontWeight.w600,
    color: ColorsApp.mainGreen,
  );

  static TextStyle font16Black400 = TextStyle(
    fontSize: 16.sp,
    fontWeight: FontWeight.w400,
    color: ColorsApp.black,
  );

  static TextStyle font16Black500WithOpacity = TextStyle(
    fontSize: 16.sp,
    fontWeight: FontWeight.w500,
    color: ColorsApp.black.withOpacity(0.5),
  );

  static TextStyle font16Gray500 = TextStyle(
      fontSize: 16.sp, fontWeight: FontWeight.w500, color: ColorsApp.gray);
  static TextStyle font16Black600 = TextStyle(
    fontSize: 16.sp,
    fontWeight: FontWeight.w600,
    color: ColorsApp.black,
  );

  static TextStyle font14Black400 = TextStyle(
    fontSize: 14.sp,
    fontWeight: FontWeight.w400,
    color: ColorsApp.black,
  );
  static TextStyle font14Black700 = TextStyle(
    fontSize: 14.sp,
    fontWeight: FontWeight.w700,
    color: ColorsApp.black,
  );

  static TextStyle font14gray700 = TextStyle(
    fontSize: 14.sp,
    fontWeight: FontWeight.w700,
    color: ColorsApp.gray,
  );
  static TextStyle font14yallow700 = TextStyle(
    fontSize: 14.sp,
    fontWeight: FontWeight.w700,
    color: ColorsApp.yallow,
  );
  static TextStyle font14Black500 = TextStyle(
    fontSize: 14.sp,
    fontWeight: FontWeight.w500,
    color: ColorsApp.gray,
  );
  static TextStyle font16Black700 = TextStyle(
    fontSize: 16.sp,
    fontWeight: FontWeight.w700,
    color: ColorsApp.black,
  );
  static TextStyle font14yallow500 = TextStyle(
    fontSize: 14.sp,
    fontWeight: FontWeight.w500,
    color: ColorsApp.yallow,
  );

  static TextStyle font12blackOpacity700 = TextStyle(
    fontSize: 12.sp,
    fontWeight: FontWeight.w700,
    color: ColorsApp.blackOpacity,
  );

  static TextStyle font12mainGreen700 = TextStyle(
    fontSize: 12.sp,
    fontWeight: FontWeight.w700,
    color: ColorsApp.mainGreen,
  );

  static TextStyle font24mainGreen700 = TextStyle(
    fontSize: 24.sp,
    fontWeight: FontWeight.w700,
    color: ColorsApp.mainGreen,
  );

  static TextStyle font12whiteOpacity700 = TextStyle(
    fontSize: 12.sp,
    fontWeight: FontWeight.w700,
    color: ColorsApp.white,
  );
  static TextStyle font12black500 = TextStyle(
    fontSize: 12.sp,
    fontWeight: FontWeight.w500,
    color: ColorsApp.black,
  );

  static TextStyle font10whiteOpacity500 = TextStyle(
    fontSize: 10.sp,
    fontWeight: FontWeight.w500,
    color: ColorsApp.white.withOpacity(0.4),
  );
  static TextStyle font14white700 = TextStyle(
    fontSize: 14.sp,
    fontWeight: FontWeight.w700,
    color: ColorsApp.white,
  );
  static TextStyle font10white500 = TextStyle(
    fontSize: 10.sp,
    fontWeight: FontWeight.w500,
    color: ColorsApp.white,
  );
}
